...
public class SwitchButton extends CompoundButton {
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            if (mThumbWidth == 0) {
                contentSize = widthSize - getPaddingLeft() - getPaddingRight();
                moveRange = floor(contentSize - Math.max(mThumbMargin.left, 0) - Math.max(mThumbMargin.right, 0));
                if (moveRange < 0) {
                    mThumbWidth = 0;
                    mBackWidth = 0;
                    return measuredWidth;
                ...
                mBackWidth = floor(moveRange + mThumbMargin.left + mThumbMargin.right);
                if (mBackWidth < 0) {
                    mThumbWidth = 0;
                    mBackWidth = 0;
                    return measuredWidth;
    ...
    private int floor(double dimen){
        return (int) Math.floor(dimen);
    }
